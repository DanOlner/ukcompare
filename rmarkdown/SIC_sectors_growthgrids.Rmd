...
Looking at percent change in jobs vs GVA for all places, centred, South Yorkshire's standout position is clear. 

-- The four quadrants cut by red lines separate which of GVA or jobs count has grown vs shrunk.

-- In The lighter diagonal area top right, any vectors there will have seen GVA per worker drop (with these numbers, cf. lack of error bars). So e.g. it's possible for both GVA and job numbers to drop, but if the proportional GVA drop is smaller, GVA per worker still rises.
...
Pre-COVID, South Yorkshire was still strong, but had a number of peers with the same magnitude (and note, none of this includes error bars).
...
Looking at the normalised percent plot for manufacturing puts South Yorkshire in the context of other places in a different way.
