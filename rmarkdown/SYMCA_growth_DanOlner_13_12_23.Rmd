...
-- Apologies: data is only for South Yorkshire, or for Sheffield vs all of Barnsley, Doncaster & Rotherham combined (limitation of GVA data only available at ITL3 geography).
...
-- Narrow remit for this work; convserations ongoing about how this all ties to broader SYMCA goals and how to measure
...
-- Shows overall picture e.g. manufacturing (top right) is largest sector for total GVA; higher increase in jobs earlier, slight decrease later (but job count data is volatile...)
...
-- Anything point into bottom right darker half saw GVA per FT worker grow
...
Note the different kinds of GVA per worker growth here: lines going 'south east' (purple labels) having growing GVA per worker with DROPPING full time worker numbers; only those going 'northeast' are growing both worker numbers AND GVA
...
## ICT pre-COVID: percentage change 2017 to 2021 for all places

Including COVID years, South Yorkshire ICT's resilience stands out relative to other places.

We'll come back to what ICT is actually composed of shortly (spoiler: mainly telecommunications sector.)
...
In this extreme example, construction and manufacturing in South Yorkshire has linear slopes added and 95% confidence intervals shown in grey. The two trends are clearly statistically separable - the range of the trend slopes within the grey bounds differs; additionally, manufacturing has a positive slope and construction is negative.
...
This plot does exactly the same thing, but for all slope pairs, for the ICT sector. Every place is compared to every other. 
...
While also true for manufacturing - as with the previous slides - that sector doesn't stand out as very different from many other growth places.
...
Arts and entertainment similarly saw GVA drops everywhere in COVID, though many places also saw very large job losses (so GVA per worker actually increased).
...
Looking at finer-grained sector data, ICT can be broken down into its subsectors (here, first and last timepoints for three year moving average, to smooth a little.)
...
Computer programming and consultancy actually has higher job growth numbers and is contributing to GVA growth through that, but (with these numbers) GVA per FT worker is lower than telecoms, and dropped.
...
## Sheffield vs Barnsley/Doncaster/Rotherham Telecommunications/ICT growth comparison
...
This plots GROWTH RATES BETWEEN YEARS (with a 7 year moving average) - the values are all positive because telecoms has grown every year.
...
Taking a closer look now at manufacturing subsectors GVA and jobs change (3 year moving averages used again) and plotting actual GVA versus jobs.
...
## A closer look at Sheffield vs Barnsley, Doncaster and Rotheram. (1) Location quotients

This plots LOCATION QUOTIENTS for Barnsley/Doncaster/Rotherham (circles, mostly green) versus Sheffield (diamonds).

LOCATION QUOTIENT (LQ): how much the proportion of sector is higher than the same proportion nationally - so, it's a measure of what sectors are concentrated here.
...
Black lines show full range of what LQ's been in the data.

Numbers for the percent of Barnsley/Doncaster/Rotherham's economy each sector represents are given on right.
...
And most sectors have a higher concentration than in Sheffield e.g. 'other manufacturing' for Barnsley/Doncaster/Rotherham is to the right of Sheffield's diamond (though looks like wasn't always the case in this data).
...
## (2a) 2017-21 GVA change comparing manufacturing in Sheffield vs Barnsley/Doncaster/Rotherham

Notes

3 yr movin av showing 2015-17 av vs 2019-2021 av
...
## (2b) 2017-21 GVA change, comparing top 15 NON-manufacturing sectors in Sheffield vs Barnsley/Doncaster/Rotherham  
...
## (2c) 2017-21 GVA change, remaining 25 sectors in Sheffield vs Barnsley/Doncaster/Rotherham  
...
The SIC (industrial) vs SOC (occupational) data includes data for setting error bars - here, this is used for 95% confidence intervals to find likely genuinely different proportions in number of 16+ employed in difference skill vs sectors.

Greener = higher proportion than comparator place
Redder = lower proportion

SY has higher proportions for many lower skilled SICSOCs / lower for many higher skilled.
