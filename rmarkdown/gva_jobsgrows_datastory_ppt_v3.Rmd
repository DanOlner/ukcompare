---
title: "GVA + jobs growth in South Yorkshire"
author: "Dan Olner"
date: "2023-10-31"
...
The geography of GVA per worker suggests large regional clusters that South Yorkshire could ride, even if not significantly strong in, such as health and scientific research + development / and other professional scientific and technical.

There are important differences between Sheffield versus Barnsley/Doncaster/Rotherham. E.g. Telecoms and IT is a strong growth sector in Sheffield. B/D/R has growth in wholesale (growing everywhere) and some select manufacturing.
...
Some of Sheffield and B/D/R’s manufacturing sectors go with the national trend of steady decline (basic and fabricated), some do not follow the same pattern (e.g. ‘other manufacturing’).

---

South Yorkshire's low skill equilibrium is visible in the SIC v SOC data, though also its skilled workers in some sectors. GM/WY/LCR comparisons show how distinct GM is.
...
This example shows how few sectors are separable (black error bars) when comparing South Yorkshire to Greater Manchester, but...
...
## South Yorkshire vs all other ITL2 regions

... that approach can be used to compare to all other regions, to look for overall patterns.
...
West Yorkshire is actually quite similar to SY, skillsw-wise, for those sectors where 95% CIs don't cross.
...
Liverpool City Region shows its higher numbers of public admin workers in higher skills bands

But lower for skilled labour.

```{r}
# comparator = 'South Yorkshire'
comparator = 'Merseyside'
# comparator = 'West Yorkshire'
...
Greater Manchester is quite different from any of those comparators.
...
# SOUTH YORKSHIRE: GVA DIRECTIONS vs UK
...
```{r}
itl2.lq <- readRDS('../data/bres_5digit_fulltime.rds') %>% 
   mutate(
    INDUSTRY_NAME_REDUCED = gsub(x = INDUSTRY_NAME, pattern = 'of |and |acture|acturing| activities| equipment| products', replacement = '')
  )
...
Next slide's table is the previous two slides' sectors, binned into SIC sections
...
sic.sections <- read_csv('../data/2digitSICcodes_w_sections.csv')
...
# GVA LOCATION QUOTIENTS: SY v GREATER MANCHESTER, LIVERPOOL CITY REGION & WEST YORKSHIRE
...
## Sheffield vs Barnsley/Doncaster/Rotherham raw GVA LQ plots #1/2

1: Sheffield raw GVA LQs (circles) comparing to B/D/R (diamonds). Things to note:

Relatively smaller number of sectors relatively concentrated in Sheffield

Quite a few with higher concentration in B/D/R.
...
## Sheffield vs Barnsley/Doncaster/Rotherham raw GVA LQ plots #2/2

2: Looking at the GVA LQ data from B/D/R perspective:

More sectors with relative raw GVA concentration in B/D/R. 

Of those, many with equivalents in Sheffield that are less concentrated - exceptions being three largely public sectors (public admin/defence, health, education) and fabricated metal products.
